...
if ( ! function_exists( 'sh_get_elementor_products_config' ) ) {
	/**
	 ...
	 * @return array
	 */
	function sh_get_elementor_products_config() {
		return array(
			// General.
			'element_title'            => '',
			...
			'pagination'               => '',
			'items_per_page'           => 12,
			'spacing'                  => 'products_spacing',
			'columns'                  => [ 'size' => 4 ],
			'products_masonry'         => 'products_masonry',
			'products_different_sizes' => 'products_different_sizes',
			'product_quantity'         => 'product_quantity',

			// Design.
			'product_hover'            => 'products_hover',
			'sale_countdown'           => 0,
...
if ( ! function_exists( 'sh_elementor_products_template' ) ) {
	function sh_elementor_products_template( $settings ) {
		if ( ! sh_woocommerce_installed() ) {
			return '';
		}

		$default_settings = sh_get_elementor_products_config();
		$settings         = wp_parse_args( $settings, $default_settings );

		if ( ! $settings['spacing'] ) {
			$settings['spacing'] = 'products_spacing';
		}
		...
		$is_ajax                    = sh_is_woo_ajax() && 'yes' !== $settings['force_not_ajax'];
		...
		if ( 'top_rated_products' === $settings['post_type'] ) {
			add_filter( 'posts_clauses', 'sh_order_by_rating_post_clauses' );
			$products = new WP_Query( apply_filters( 'sh_product_element_query_args', $query_args ) );
			remove_filter( 'posts_clauses', 'sh_order_by_rating_post_clauses' );
		} else {
			$products = new WP_Query( apply_filters( 'sh_product_element_query_args', $query_args ) );
		}
		...
		if ( 'inherit' === $settings['product_hover'] ) {
			$settings['product_hover'] = 'products_hover';
		}

		// Loop settings.
		wc_set_loop_prop( 'timer', $settings['sale_countdown'] );
		wc_set_loop_prop( 'progress_bar', $settings['stock_progress_bar'] );
		wc_set_loop_prop( 'product_hover', $settings['product_hover'] );
		wc_set_loop_prop( 'products_view', $settings['layout'] );
		wc_set_loop_prop( 'is_shortcode', true );
		wc_set_loop_prop( 'img_size', $settings['img_size'] );
		wc_set_loop_prop( 'img_size_custom', $settings['img_size_custom'] );

		if ( isset( $settings['columns']['size'] ) ) {
			sh_set_loop_prop( 'products_columns', $settings['columns']['size'] );
		}
		if ( $settings['products_masonry'] ) {
			sh_set_loop_prop( 'products_masonry', 'enable' === $settings['products_masonry'] );
		}
		if ( $settings['products_different_sizes'] ) {
			sh_set_loop_prop( 'products_different_sizes', 'enable' === $settings['products_different_sizes'] );
		}
		if ( $settings['product_quantity'] ) {
			sh_set_loop_prop( 'product_quantity', 'enable' === $settings['product_quantity'] );
		}
		if ( 'arrows' !== $settings['pagination'] ) {
			sh_set_loop_prop( 'woocommerce_loop', $settings['items_per_page'] * ( $paged - 1 ) );
		}
		if ( isset( $_GET['shop_view'] ) && $settings['shop_tools'] ) { // phpcs:ignore
			sh_set_loop_prop( 'products_view', wc_clean( wp_unslash( $_GET['shop_view'] ) ) ); // phpcs:ignore
		}

		if ( isset( $_GET['per_row'] ) && $settings['shop_tools'] ) { // phpcs:ignore
			sh_set_loop_prop( 'products_columns', wc_clean( wp_unslash( $_GET['per_row'] ) ) ); // phpcs:ignore
		...
		if ( 'carousel' === $settings['layout'] ) {
			...
			sh_enqueue_inline_style( 'product-loop' );
	
			return sh_generate_posts_slider( $settings, $products );
		}
		...
		if ( sh_loop_prop( 'product_quantity' ) ) {
			$wrapper_classes .= ' wd-quantity-enabled';
		}
		if ( 'none' !== sh_get_opt( 'product_title_lines_limit' ) && 'list' !== $settings['layout'] ) {
			$wrapper_classes .= ' title-line-' . sh_get_opt( 'product_title_lines_limit' );
		}
		if ( sh_loop_prop( 'products_masonry' ) ) {
			$wrapper_classes .= ' grid-masonry';
			wp_enqueue_script( 'imagesloaded' );
			sh_enqueue_js_library( 'isotope-bundle' );
			sh_enqueue_js_script( 'shop-masonry' );
		}

		$products_element_classes .= $settings['highlighted_products'] ? ' wd-highlighted-products' : '';
		$products_element_classes .= $settings['highlighted_products'] ? sh_get_old_classes( ' sh-highlighted-products' ) : '';
		...
		if ( 'yes' === $settings['lazy_loading'] ) {
			...
			sh_lazy_loading_init( true );
			sh_enqueue_inline_style( 'lazy-loading' );
		}
			...
			<?php if ( $settings['shop_tools'] ) : ?>
				<div class="shop-loop-head">
					<div class="wd-shop-tools">
						<?php sh_products_per_page_select( true ); ?>
						<?php sh_products_view_select( true ); ?>
	...
	<?php endif; ?>
		...
		<?php if ( $products->max_num_pages > 1 && ! $is_ajax && $settings['pagination'] ) : ?>
			...
			<?php sh_enqueue_js_script( 'products-load-more' ); ?>
			...
			<div class="products-footer">
				<?php if ( 'more-btn' === $settings['pagination'] || 'infinit' === $settings['pagination'] ) : ?>
					<a href="#" rel="nofollow noopener" class="btn wd-load-more wd-products-load-more load-on-<?php echo 'more-btn' === $settings['pagination'] ? 'click' : 'scroll'; ?>"><span class="load-more-label"><?php esc_html_e( 'Load more products', 'sh' ); ?></span></a>
					<div class="btn wd-load-more wd-load-more-loader"><span class="load-more-loading"><?php esc_html_e( 'Loading...', 'sh' ); ?></span></div>
